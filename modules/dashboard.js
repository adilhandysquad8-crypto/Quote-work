...
function renderSalesDashboard() {
  ...
  const myJobs     = d.jobs.filter(j => j.created_by === STATE.profile?.id || d.leads.some(l => l.assigned_to === STATE.profile?.id && l.id === j.lead_id));
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      ${activeJobs.length === 0
        ...
        : `<div>${activeJobs.map(j => {
          ...
          return `
          <div style="border-bottom:1px solid var(--gray-100);padding:14px 16px" onclick="openJobDetail('${j.id}')" style="cursor:pointer">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
              <div>
                <div style="font-weight:600;font-size:14px">${esc(j.customer_name||'—')}</div>
                <div style="font-size:12px;color:var(--gray-500)">${esc(j.location_text||'')} ${j.location_link?`<a href="${j.location_link}" target="_blank" onclick="event.stopPropagation()" style="color:var(--blue-600)">📍 Map</a>`:''}</div>
              </div>
        ...
        ${myVisits.map(v => {
          ...
          return `<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid var(--gray-100)">
            <div>
              ...
              <div style="font-size:12px;color:var(--gray-600);margin-top:2px">${esc(job?.description||'')}</div>
            </div>
